...
describe('Userinfo e2e test', () => {
    ...
    it('should load Userinfos', () => {
        entityMenu.click();
        element.all(by.css('[routerLink="userinfo"]')).first().click().then(() => {
            const expectVal = /Userinfos/;
    ...
    it('should load create Userinfo dialog', function () {
        element(by.css('button.create-userinfo')).click().then(() => {
            const expectVal = /Create or edit a Userinfo/;
