...
export class MedicalHistoryComponent implements OnInit, OnDestroy {
    ...
    loadAll () {
        this.medicalHistoryService.query({
            page: this.page,
    ...
    registerChangeInMedicalHistories() {
        this.eventSubscriber = this.eventManager.subscribe('medicalHistoryListModification', (response) => this.reset());
    }
    ...
    private onSuccess(data, headers) {
        ...
        for (let i = 0; i < data.length; i++) {
            this.medicalHistories.push(data[i]);
        }
    }
